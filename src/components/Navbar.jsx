...
const Navbar = () => {
  ...
  return (
    <nav className='flex justify-between items-center h-24 max-w-[auto] text-[#915634]'>
      <div className='ml-4'>
      ...
      <form onSubmit={handleSubmit} className='flex items-center'>
        <input  
          type='text' 
          className='rounded-xl text-black border border-2 border-[#915634] font-bold p-2 rounded' 
          placeholder="Введіть текст" 
          value={inputValue} 
          onChange={(e)  => setInputValue(e.target.value)}
        />
        <button type="submit" className='rounded-xl m-2 p-2 text-white bg-[#915634] font-bold rounded hover:bg-[#7b492b] active:bg-[#653c25]' >
          Відправити
        </button>
      ...
      <ul className={nav ? 'fixed left-0 top-0 w-[35%] h-full border-[#ffd3ba] bg-[#ffd3ba] ease-in-out duration-300 z-50' : 'ease-in-out duration-300 fixed left-[-100%] z-50'}>
